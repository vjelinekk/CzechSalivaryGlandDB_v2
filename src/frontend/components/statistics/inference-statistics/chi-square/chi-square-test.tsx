...
const ChiSquare: React.FC<ChiSquareProps> = (props) => {
    ...
    const [rowSelectedCategories, setRowSelectedCategories] = useState<Record<number, Record<InferenceChiSquareCategories, string[]>>>({});
    const [columnSelectedCategories, setColumnSelectedCategories] = useState<Record<number, Record<InferenceChiSquareCategories, string[]>>>({});
    ...
    const updateMatrix = (newRows: number, newColumns: number) => {
        ...
        for (let i = 0; i < newRows; i++) {
            ...
            for (let j = 0; j < newColumns; j++) {
                const existingValue = i < matrix.length && j < matrix[i].length ? matrix[i][j] : 0;
        ...
        <Box sx={{ p: 3 }}>
            ...
            <Paper elevation={3} sx={{ p: 3 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'center', mb: 2 }}>
                    <Grid container spacing={1} sx={{ maxWidth: '100%', overflow: 'auto' }}>
                        <Grid container item key="column-headers" spacing={1}>
                            <Grid item>
                                <Box sx={{ width: '100px', height: '56px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    {/* Empty cell */}
                                </Box>
                            </Grid>
                            ...
                            {Array.from({ length: columns }).map((_, colIndex) => (
                                <Grid item key={`column-header-${colIndex}`}>
                                    <Box sx={{
                                        width: '100px',
                                        height: '56px',
                                        display: 'flex',
                        ...
                        {matrix.slice(0, Math.min(rows, matrix.length - 1)).map((row, rowIndex) => (
                            <Grid container item key={`row-${rowIndex}`} spacing={1}>
                                <Grid item>
                                    <Box sx={{
                                        width: '100px',
                                        height: '56px',
                                ...
                                <Grid item key={`row-sum-${rowIndex}`}>
                                    <TextField
                                        ...
                                        variant="outlined"
                                        size="small"
                                        value={row[Math.min(columns, row.length - 1)] || 0}
                                        sx={{ 
                                            width: '100px', 
                                            backgroundColor: 'rgba(0, 0, 0, 0.05)',
                        ...
                        <Grid container item key="column-sums-row" spacing={1}>
                            ...
                            {matrix[matrix.length - 1]?.slice(0, columns).map((sum, colIndex) => (
                            ...
                            <Grid item>
                                <TextField
                                    ...
                                    variant="outlined"
                                    size="small"
                                    value={matrix[matrix.length - 1]?.[matrix[0]?.length - 1] || 0}
                                    sx={{ 
                                        width: '100px',
                ...
                <Box sx={{ mt: 3, display: 'flex', justifyContent: 'left', alignItems: 'center', gap: 2 }}>
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12}>
                            <Box sx={{ 
                                ...
                                mb: 2, 
                                borderRadius: 1, 
                                bgcolor: testResults.isSignificant ? 'rgba(76, 175, 80, 0.1)' : 'rgba(244, 67, 54, 0.1)',
                                border: testResults.isSignificant ? '1px solid rgba(76, 175, 80, 0.5)' : '1px solid rgba(244, 67, 54, 0.5)'
                            }}>
                                <Typography variant="h6" sx={{ fontWeight: 'bold', color: testResults.isSignificant ? '#1b5e20' : '#b71c1c' }}>
                                    {testResults.isSignificant 
                                        ? 'Výsledek je statisticky signifikantní' 
                                        : 'Výsledek není statisticky signifikantní'}
                                </Typography>
                                <Typography variant="body2">
                                    Na hladině významnosti {typeof alpha === 'number' ? alpha : 5}%.
                                </Typography>
                            </Box>
                        </Grid>
                        ...
                        <Grid item xs={12} sm={6} md={3}>
                            ...
                            <Typography variant="body1" sx={{ fontWeight: 'bold' }}>
                                {testResults.pValue && testResults.pValue < 0.0001 
                                    ? '<0.0001' 
                        ...
                        <Grid item xs={12} sx={{ mt: 2 }}>
                            <Typography variant="body2">
                                {testResults.isSignificant 
                                    ? `Hodnota Chi-kvadrát (${testResults.chiSquared?.toFixed(4)}) je větší než kritická hodnota (${testResults.criticalValue?.toFixed(4)}), proto zamítáme nulovou hypotézu.`
                                    : `Hodnota Chi-kvadrát (${testResults.chiSquared?.toFixed(4)}) je menší než kritická hodnota (${testResults.criticalValue?.toFixed(4)}), proto nelze zamítnout nulovou hypotézu.`
                                }
                            </Typography>
                        </Grid>
                    </Grid>
