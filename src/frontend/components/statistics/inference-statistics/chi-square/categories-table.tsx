...
interface CategoriesTableProps {
    ...
    selectedCategories?: Record<number, Record<InferenceChiSquareCategories, string[]>>;
...
const CategoriesTable: React.FC<CategoriesTableProps> = ({
    ...
    const isItemSelected = (itemKey: string, categoryIndex: number): boolean => {
        return selectedCategories[categoryIndex]?.[activeTab]?.includes(itemKey) || false;
    };
            ...
            {categories.map((itemKey, itemIndex) => (
                <TableRow key={`row-${itemIndex}`} sx={{
                    ...
                    {[...Array(numberOfCategories)].map((_, categoryIndex) => (
                        <TableCell key={`cell-${itemIndex}-${categoryIndex}`} align="center" sx={{ py: 1 }}>
                            <Checkbox
                                size="small"
                                checked={isItemSelected(itemKey, categoryIndex)}
                                onChange={() => onCheckboxChange(itemKey, categoryIndex)}
                            />
                        </TableCell>
                    ))}
                </TableRow>
