...
describe('PersonalData component', () => {
    test('renders the section headings and form inputs', () => {
        ...
        const demographicInfoCheckbox = screen.getByText('Demografické informace - kraj trvalého bydliště pacienta');
        expect(demographicInfoCheckbox).toBeInTheDocument();

        const otherTumorsCheckbox = screen.getByText('Jiné nádorové onemocnění v OA');
        expect(otherTumorsCheckbox).toBeInTheDocument();

        const otherSalivaryCheckbox = screen.getByText('Jiné onemocnění velkých slinných žláz v OA');
