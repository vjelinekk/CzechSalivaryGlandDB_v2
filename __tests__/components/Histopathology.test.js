...
describe('Histopathology component', () => {
  ...
  test('renders section headings and various input components', () => {
    render(<Histopathology formData={formData} setFormData={setFormData} disabled={false} />);
    ...
    expect(subsectionHeadings[0]).toHaveTextContent('Histologický typ nádoru');
    expect(subsectionHeadings[1]).toHaveTextContent('Specifikace histologického typu');

    // Assert ConditionalCheckboxes and their options
    const conditionalCheckboxes = screen.getAllByTestId('conditional-checkboxes');
    ...
    const numberInput = screen.getByText('Velikost nádoru - největší rozměr (mm):'); // Text label expected
    expect(numberInput).toBeVisible();

    // Assert TextInput component
    const textInput = screen.getByText('Velikost nádoru - nebyla určena (vysvětlit):');
  ...
  test('disables input components when disabled prop is true', () => {
    render(<Histopathology formData={formData} setFormData={setFormData} disabled={true} />);

    // Assert all input components are disabled
    const inputs = screen.getAllByRole('textbox').concat(screen.getAllByRole('checkbox'));
