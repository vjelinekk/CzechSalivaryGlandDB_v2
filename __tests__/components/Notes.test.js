...
describe('Notes component', () => {
  test('renders the section heading and textarea input', () => {
    ...
    render(
      <Notes formData={formData} setFormData={setFormData} disabled={false} />
    );

    // Assert section heading
    const sectionHeading = screen.getByRole('heading', { level: 1, name: /poznámky/i });
    ...
    expect(setFormData).toHaveBeenCalledWith({ ...formData, poznamky: 'Updated notes' });
  });

  test('renders the section heading and empty textarea input when formData is null', () => {
    // Render the component with null formData
    render(<Notes formData={null} setFormData={jest.fn()} disabled={false} />);
  ...
  test('disables textarea input when disabled prop is true', () => {
    ...
    render(<Notes formData={null} setFormData={jest.fn()} disabled={true} />);
